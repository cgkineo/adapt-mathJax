...
Both live in `js/MathJaxLoader.js` (`DEFAULT_CONFIG`) and are mirrored in `schema/config.schema.json` and `properties.schema`:

- `loader.paths.mathjax` → `libraries/mathjax/4`
...
3. If the major version changed, update `LIBRARY_PATH` in `js/MathJaxLoader.js` and the mirrored defaults in both schema files.
